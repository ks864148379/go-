...
func (server *CenterServer)addPlayer(params string) error {
	...
	err := json.Unmareshal([]byte(params), &player)
...
func (server *CenterServer)listPlayer(params string) (players string, err error) {
	...
	if len(server.mutex.players) > 0 {
		b, _ := json.Marshal(server.players)
		players = string(b)
	} else {
		err = errors.New("No players")
	}
	return
}
...
func (server *CenterServer)broadcast(params string) error {
	...
	err := json.Unmareshal([]byte(params), &message)
